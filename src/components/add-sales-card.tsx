...
export function AddSalesCard({ onSaleAdded }: AddSalesCardProps) {
  ...
  const fetchSkuOptions = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Sale SKU API Response:', data) // Debug log to see the actual response structure
        ...
        if (Array.isArray(data)) {
          ...
          console.warn('Sale SKU API response is not in expected format:', data)
          setSkuOptions([])
          setShowSkuDropdown(false)
          return
        }
        ...
        console.error('Sale SKU API request failed:', response.status)
        setSkuOptions([])
        setShowSkuDropdown(false)
      }
    } catch (error) {
      console.error('Error fetching Sale SKU options:', error)
      setSkuOptions([])
      setShowSkuDropdown(false)
    }
  }, [])
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Record a new sale transaction and update stock levels
        </CardDescription>
      </CardHeader>
      <CardContent>
